...
int main() {
    ...
    int res3=write_block(memory,trustedArea,0,block_to_write2,BLOCK_SIZE);
    ...
    read_block(memory,trustedArea,0,buf);
...
//    getRoot(memory,trustedArea,root2);
    ...
    int res=write_block(memory,trustedArea,0,block_to_write3,BLOCK_SIZE);
    read_block(memory,trustedArea,0,buffer1);
    getRoot(memory,trustedArea,root2);
    cmp=m_strncmp(buffer1,block_to_write3,BLOCK_SIZE);
    assert(cmp==0 && res==0);
    std::cout<<"--Write + Read Succeeded"<<std::endl;
    res=write_block(memory,trustedArea,0,block_to_write4,BLOCK_SIZE);
    read_block(memory,trustedArea,0,buffer2);
    cmp=m_strncmp(buffer2,block_to_write4,BLOCK_SIZE);
    assert(cmp==0);
    std::cout<<"--Write + Read Succeeded"<<std::endl;
    res=write_block(memory,trustedArea,1,block_to_write5,BLOCK_SIZE);
    read_block(memory,trustedArea,1,buffer3);
    cmp=m_strncmp(buffer3,block_to_write5,BLOCK_SIZE);
    assert(cmp==0 && res==0);
    std::cout<<"--Write + Read Succeeded"<<std::endl;
    res=write_block(memory,trustedArea,2,block_to_write6,BLOCK_SIZE);
    read_block(memory,trustedArea,2,buffer4);
    cmp=m_strncmp(buffer4,block_to_write6,BLOCK_SIZE);
    assert(cmp==0 && res==0);
    std::cout<<"--Write + Read Succeeded"<<std::endl;
    res=write_block(memory,trustedArea,3,block_to_write7,BLOCK_SIZE);
    read_block(memory,trustedArea,3,buffer1);
    cmp=m_strncmp(buffer1,block_to_write7,BLOCK_SIZE);
    assert(cmp==0 && res==0);
    std::cout<<"--Write + Read Succeeded"<<std::endl;
    res=write_block(memory,trustedArea,4,block_to_write8,BLOCK_SIZE);
    read_block(memory,trustedArea,4,buffer1);
    cmp=m_strncmp(buffer1,block_to_write8,BLOCK_SIZE);
    assert(cmp==0 && res==0);
    std::cout<<"--Write + Read Succeeded"<<std::endl;
    res=write_block(memory,trustedArea,6,block_to_write9,BLOCK_SIZE);
    read_block(memory,trustedArea,6,buffer1);
    ...
    res=verify_integrity(memory,trustedArea);                   // Verify shall fail
    assert(res==0);
    std::cout<<"--HMAC attack detected :) Tree works fine:)";
    res=read_block(memory,trustedArea,4,buffer1);           //Tree is not good, any read shall fail
    res2=write_block(memory,trustedArea,7,block_to_write5,BLOCK_SIZE);// any write shall fail as well
    assert(res==-1 && res2==-1);
    ...
    getRoot(memory,trustedArea,root3);
    int read_res=read_block(memory,trustedArea,0,buffer1);
    ...
    res=read_block(memory,trustedArea,0,buffer3);           //Tree is not good, any read shall fail
    ...
    getRoot(memory,trustedArea,root);
    trustedArea->update_root(root);     // Now tree is fixed
    res=verify_integrity(memory,trustedArea);
    assert(res==1);
    res=write_block(memory,trustedArea,0,block_to_write4,BLOCK_SIZE);
    std::cout<<"--After fixing the root, Tree is good now \n";
...
int main2(){
    ...
    unsigned char root[SHA_LENGTH_BYTES];
    getRoot(mainMemory,trustedArea,root);
    trustedArea->update_root(root);
    ////////////////////////////////////////// Testing In 1,2,3 ...  ////////////////////////////////////////////////
